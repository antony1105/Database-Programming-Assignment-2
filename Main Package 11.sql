...
AS
  ...
  IS
    ...
    l_column_headings VARCHAR2(100) := rpad('Merchant ID', 15, l_indicator) || rpad('Merchant Name', 30, l_indicator)
  ...
  BEGIN
    ins_file('DS_' || l_date || '_WT', get_deskbank_header, get_deskbank_body, get_deskbank_footer);
  EXCEPTION
    WHEN OTHERS
    THEN
  ...
  BEGIN
    ins_file(l_file_name, get_settlement_report_header(p_date, l_length), get_settlement_report_body(p_date), 'get_settlement_footer');
    --dbms_output.put_line(owa_util.get_procedure);
  COMMIT;
  EXCEPTION
    WHEN OTHERS
    THEN
